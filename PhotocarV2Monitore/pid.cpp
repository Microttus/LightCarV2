...
float PID::calculate(float value, float target){
  double dt = (millis()*0.001) - _mark_time;
  _mark_time = (millis()*0.001);
...
float PID::calcsaturation(float value, float target, float saturationMin, float saturationMax){
    ...
    _sat_calc_val = constrains(calc_val, saturationMin, saturationMax);
    _last_i_val = float(int_error);
    _last_val = value;
...
float PID::constrains(float value, float saturationMin, float saturationMax) {
